...
function Calendar({ value, onChange }) {
  ...
  return (
    <div className="border border-[#0B1B3D]/10 p-5 bg-white" data-testid="calendar">
      <div className="flex items-center justify-between mb-4">
        <button onClick={() => setMonth(new Date(month.getFullYear(), month.getMonth() - 1, 1))} className="p-2 text-[#0B1B3D] hover:text-[#C5A059]" data-testid="cal-prev">‹</button>
        <div className="font-serif text-lg text-[#0B1B3D]">{MONTHS[month.getMonth()]} {month.getFullYear()}</div>
        <button onClick={() => setMonth(new Date(month.getFullYear(), month.getMonth() + 1, 1))} className="p-2 text-[#0B1B3D] hover:text-[#C5A059]" data-testid="cal-next">›</button>
      </div>
      <div className="grid grid-cols-7 gap-1 text-center text-xs text-[#64748B] mb-2">
          ...
          return (
            <button
              ...
              className={`aspect-square text-sm transition-colors ${
                isSel ? "bg-[#0B1B3D] text-white" : sel ? "text-[#0B1B3D] hover:bg-[#C5A059]/20" : "text-[#0B1B3D]/20 cursor-not-allowed"
              }`}
...
export default function AppointmentPage() {
  ...
  if (confirmed) {
    ...
    return (
      <div className="max-w-2xl mx-auto px-6 py-24 text-center" data-testid="appointment-confirmed">
        ...
        <h1 className="font-serif text-5xl text-[#0B1B3D] mb-6">¡Te esperamos!</h1>
        <div className="bg-white border border-[#0B1B3D]/10 p-8 text-left space-y-3 mb-8">
          <div className="flex justify-between"><span className="text-[#64748B]">Servicio</span><strong className="text-[#0B1B3D]">{confirmed.service}</strong></div>
          <div className="flex justify-between"><span className="text-[#64748B]">Fecha</span><strong className="text-[#0B1B3D]">{confirmed.date}</strong></div>
          <div className="flex justify-between"><span className="text-[#64748B]">Hora</span><strong className="text-[#0B1B3D]">{confirmed.time}</strong></div>
          <div className="flex justify-between"><span className="text-[#64748B]">A nombre de</span><strong className="text-[#0B1B3D]">{confirmed.customer_name}</strong></div>
        </div>
        <p className="text-sm text-[#64748B] mb-6">Te contactaremos pronto para confirmar. También puedes confirmar por WhatsApp:</p>
  ...
  return (
    <div className="max-w-7xl mx-auto px-6 md:px-12 lg:px-16 py-16 md:py-24" data-testid="appointment-page">
      ...
      <h1 className="font-serif text-5xl md:text-6xl text-[#0B1B3D] gold-rule mb-12">Reserva en línea</h1>
      ...
      <form onSubmit={submit} className="grid lg:grid-cols-2 gap-10">
        <div className="space-y-8">
            ...
            <div className="grid sm:grid-cols-2 gap-3" data-testid="service-list">
              {services.map((s) => (
                <button
                  ...
                  className={`p-4 border text-left transition-colors ${service === s.name ? "border-[#C5A059] bg-[#C5A059]/5" : "border-[#0B1B3D]/15 hover:border-[#C5A059]"}`}
                  data-testid={`service-${s.name}`}
                >
                  <div className="font-serif text-base text-[#0B1B3D]">{s.name}</div>
                  <div className="text-xs text-[#64748B] mt-1 flex items-center gap-1"><Clock className="w-3 h-3" /> {s.duration_min} min</div>
                </button>
              ))}
            </div>
          </div>
          ...
          <div>
            ...
            <p className="text-xs text-[#64748B] mt-2">Lunes a Sábado · 9:00 a 14:00 y 15:00 a 19:00 · Domingos cerrado.</p>
          </div>
        </div>
        ...
        <div className="space-y-8">
          <div>
            ...
            {!date && <p className="text-sm text-[#64748B]">Selecciona primero una fecha.</p>}
            ...
            {date && !closed && (
              <div className="grid grid-cols-4 sm:grid-cols-5 gap-2" data-testid="time-slots">
                {slots.length === 0 ? (
                  <p className="col-span-full text-sm text-[#64748B]">Sin disponibilidad ese día.</p>
                ) : slots.map((t) => (
                  <button
                    key={t}
                    type="button"
                    onClick={() => setTime(t)}
                    className={`py-2 text-sm border transition-colors ${time === t ? "bg-[#0B1B3D] text-white border-[#0B1B3D]" : "border-[#0B1B3D]/15 text-[#0B1B3D] hover:border-[#C5A059]"}`}
                    data-testid={`slot-${t}`}
                  >
                    {t}
                  </button>
          ...
          <div>
            ...
            <div className="space-y-3">
              <input required placeholder="Nombre completo" className="w-full border border-[#0B1B3D]/20 px-4 py-3 focus:border-[#C5A059] focus:outline-none"
                value={form.customer_name} onChange={(e) => setForm({ ...form, customer_name: e.target.value })} data-testid="appt-name" />
              <input required placeholder="Teléfono / WhatsApp" className="w-full border border-[#0B1B3D]/20 px-4 py-3 focus:border-[#C5A059] focus:outline-none"
                value={form.customer_phone} onChange={(e) => setForm({ ...form, customer_phone: e.target.value })} data-testid="appt-phone" />
              <input type="email" placeholder="Email (opcional)" className="w-full border border-[#0B1B3D]/20 px-4 py-3 focus:border-[#C5A059] focus:outline-none"
                value={form.customer_email} onChange={(e) => setForm({ ...form, customer_email: e.target.value })} data-testid="appt-email" />
              <textarea rows={3} placeholder="Notas adicionales (opcional)" className="w-full border border-[#0B1B3D]/20 px-4 py-3 focus:border-[#C5A059] focus:outline-none resize-none"
