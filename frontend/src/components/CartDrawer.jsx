...
export default function CartDrawer() {
  ...
  return (
    <div className="fixed inset-0 z-50" data-testid="cart-drawer">
      <div className="absolute inset-0 bg-[#701A3D]/50" onClick={() => setOpen(false)} />
      <aside className="absolute right-0 top-0 h-full w-full max-w-md bg-white flex flex-col">
        <header className="px-6 py-5 border-b border-[#701A3D]/10 flex items-center justify-between">
          <div>
            <div className="eyebrow text-[#C5A059]">Carrito</div>
            <h2 className="font-serif text-2xl text-[#701A3D]">Tu selección</h2>
          </div>
          <button onClick={() => setOpen(false)} data-testid="cart-close" aria-label="Cerrar">
            <X className="w-5 h-5 text-[#701A3D]" />
          </button>
        </header>
        ...
        <div className="flex-1 overflow-y-auto">
          {items.length === 0 ? (
            <div className="p-12 text-center">
              <p className="text-[#374151] mb-6">Tu carrito está vacío.</p>
              <button className="btn-outline-navy text-sm" onClick={() => { setOpen(false); nav("/catalogo"); }} data-testid="cart-empty-browse">
                Explorar catálogo
              </button>
            </div>
          ) : (
            <ul className="divide-y divide-[#701A3D]/10">
              {items.map((i) => (
                <li key={i.id} className="p-6 flex gap-4" data-testid={`cart-item-${i.id}`}>
                  <img src={i.image} alt={i.name} className="w-20 h-20 object-cover bg-[#FCE7F3]" />
                  <div className="flex-1">
                    <h4 className="font-serif text-[#701A3D] leading-tight">{i.name}</h4>
                    <div className="text-xs text-[#374151] mb-2">{i.style} · {i.type}</div>
                    <div className="flex items-center gap-2">
                      <button className="w-7 h-7 border border-[#701A3D]/20 text-[#701A3D]" onClick={() => update(i.id, i.quantity - 1)}>−</button>
                      <span className="text-sm w-6 text-center" data-testid={`cart-qty-${i.id}`}>{i.quantity}</span>
                      <button className="w-7 h-7 border border-[#701A3D]/20 text-[#701A3D]" onClick={() => update(i.id, i.quantity + 1)}>+</button>
                    </div>
                  </div>
                  <div className="text-right flex flex-col justify-between">
                    <div className="font-serif text-[#701A3D]">${(i.price * i.quantity).toLocaleString("es-MX")}</div>
                    <button onClick={() => remove(i.id)} className="text-[#374151] hover:text-red-500" data-testid={`cart-remove-${i.id}`}>
        ...
        {items.length > 0 && (
          <div className="border-t border-[#701A3D]/10 p-6 space-y-4">
            <div className="flex justify-between">
              <span className="text-sm text-[#374151]">Total</span>
              <span className="font-serif text-2xl text-[#701A3D]" data-testid="cart-total">${total.toLocaleString("es-MX")} MXN</span>
            </div>
            <button
            ...
            <button onClick={clear} className="w-full text-xs text-[#374151] hover:text-[#701A3D]" data-testid="cart-clear">
              Vaciar carrito
            </button>
          </div>
        )}
